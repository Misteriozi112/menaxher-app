...
//... brenda bindEvents()
$('#minimize-btn')?.addEventListener('click', () => window.api.windowControls('minimize'));
$('#maximize-btn')?.addEventListener('click', () => window.api.windowControls('maximize'));
$('#close-btn')?.addEventListener('click', () => window.api.windowControls('close'));
    // Theme Toggle
...
function searchAll(q) {
  ...
  const punetoretByName = state.punetoret.reduce((acc, p) => {
      acc[p.emri] = p; // Keep the latest record only
      return acc;
  }, {});
